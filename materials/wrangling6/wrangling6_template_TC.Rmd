...
start_date <- tidyraw %>%
  ...
  mutate(value = str_trim(value, "both")) %>% # trim the white space
  mutate(value = str_replace(value, "\\,.*", "")) %>% # remove everything that follows the first comma
  mutate(value = str_replace(value, "\\-.*", "")) %>% # remove everything that follows the first dash
  slice(3:n()) # slice off the vector, keeping only the elements from position 3 onwards

dates <- rep(start_date$value, each=3) # use the replication function to repeat each element in the vector three times

labels <-rep(c("students", "staff", "clusters"), nrow(start_date))

# paste together my dates and labels
...
```{r, eval=FALSE}
  raw %>%
    setNames(c("code", "site_name", dates_labels)) # setNames() takes a df and gives it some new variable names
```
