---
title: "#TidyTuesday"
subtitle: "Class Week 4 / TT Week 5"
author: "Tamsin Chen"
...
# load packages
  ...
  library(eeptools)
  library(gt)
  library(janitor) # at Sandy's suggestion
  library(snakecase)
...
# check variable types and check which column uniquely identifies the breed_traits and breed_rank_all data frames
glimpse(breed_traits)
glimpse(breed_rank_all)
glimpse(trait_description)
isid(breed_traits, vars = c("Breed")) # TRUE
isid(breed_rank_all, vars = c("Breed")) # TRUE
```

After browsing the data, I am interested in building a table of the 10 worst dog breeds in 2020 and some of their interesting traits. My past Tidy Tuesdays have been plots of some sort -- never tables. I'm keen to try out the gt package to make tables. Full disclosure -- I am struggling with a really tight time constraint for this Tidy Tuesday. I'm going to challenge myself to do things FAST.

```{r data prep}

breed_traits_cleaned <- breed_traits %>%
  clean_names() %>%
  mutate(breed = as.character(breed)) %>%
  mutate(breed = to_snake_case(breed))

breed_rank_all_cleaned <- breed_rank_all %>%
  clean_names() %>%
  mutate(breed = as.character(breed)) %>%
  mutate(breed = to_snake_case(breed))

rm(breed_traits, breed_rank_all)

rank_and_traits <- left_join(breed_rank_all_cleaned, breed_traits_cleaned, by = "breed") %>% # merge breed_rank_all and breed_traits on breed
  filter(x2020_rank > 185) # filter to the 10 worst dog breeds
```
